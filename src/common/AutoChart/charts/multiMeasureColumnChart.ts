...
const getChartSpec = (
  data: GetChartConfigProps['data'],
  dataProps: GetChartConfigProps['dataProps'],
) => {
  try {
    const field4Y = dataProps?.filter((field) =>
      // @ts-ignore
      hasSubset(field.levelOfMeasurements, ['Interval']),
    );
...
/* 订制一个图表需要的所有参数 */
export const multiMeasureColumnChart: CustomChart = {
  /* 图表唯一 Id */
...
export default multiMeasureColumnChart;
